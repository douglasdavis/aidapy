...
def hplot_mpl(root_file, hist_name='met_1pj', xtitle='', ytitle='',logy=False,
              proc_names=['Wt','ttbar','Fakes','WW','Diboson','Zjets']):
    ...
    data     = root_file.Get('AIDA_nominal/'+'Data_'+hist_name)
    data     = hist2array(data)
    nom_h, total_band, edges = total_systematic_histogram(root_file,hist_name,proc_names)
    centers = np.delete(edges,[0])-(np.ediff1d(edges)/2.0)
    to_stack = [nominals[name][0] for name in ['Diboson','Fakes','WW','Wt','Zjets','ttbar']]
    ...
    ax.hist([centers for _ in to_stack],weights=to_stack,bins=edges,stacked=True,
            ...
            label=[r'$\mathrm{Diboson}$',r'$\mathrm{Fakes}$','$WW$',r'$Wt$',r'$Z+\mathrm{jets}$',r'$t\bar{t}$'])
    ...
    trashpatch = patches.Rectangle((0,0),0,0,hatch='\\\\\\\\',fill=False,edgecolor='none',
                                   label=r'$\mathrm{Systematics}$')
    ax.add_patch(trashpatch)
    ax.errorbar(centers,data,yerr=np.sqrt(data),fmt='ko')
    ax.legend(numpoints=1)
    ax.set_ylim([0,np.max(data)*1.3])
    ax.text(.05,.92,'ATLAS',transform=ax.transAxes,style='oblique',fontproperties=fontATLAS,size=16)
    ax.text(.175,.92,r'Internal, AIDA $e^\pm\mu^\mp$, pre-fit',transform=ax.transAxes,size=16)
    ax.text(.05,.835,r'$\sqrt{s}=13\,\mathrm{TeV},\,\int \mathcal{L} = 36.1\,\mathrm{fb}^{-1}$',transform=ax.transAxes,size=16)
    ax.text(.05,.75,'',transform=ax.transAxes,size=16)
    axerr.errorbar(centers,data/nom_h,fmt='ko')#data/(nom_h*nom_h)*total_band
    errpatches = []
